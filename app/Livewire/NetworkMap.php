...
class NetworkMap extends Component
{
    ...
    public function render() {
        ...
        $routerNodes = $routers->whereNotNull('latitude')->whereNotNull('longitude')->map(fn($r)=>[
            'lat'=>(float)$r->latitude,'lng'=>(float)$r->longitude,'id'=>$r->id,'label'=>$r->router_name,
            'ip'=>$r->ip_address,'location'=>$r->location,'status'=>$r->action === 'connected' ? 'online' : 'offline','kind'=>'router','router'=>$r->router_name,
        ])->values()->toArray();
        $deviceLocations = NetworkInventoryDevice::query()
            ->whereIn('type',['olt','onu','switch','wifi-router','access-point'])
            ->whereNotNull('latitude')->whereNotNull('longitude')->get()
            ->map(fn($d)=>['lat'=>(float)$d->latitude,'lng'=>(float)$d->longitude,'id'=>$d->id,'label'=>$d->name,'status'=>$d->health_status ?: ($d->status ?: 'unknown'),'router'=>null,'kind'=>$d->type,'ip'=>$d->ip_address,'location'=>$d->location])
            ->values()->toArray();
        $customers = CustomersInfo::active()->count();
        $nodes = CustomersAddress::query()->leftJoin('customers_infos as ci','ci.customer_unique_id','=','customers_addresses.customer_address_unique_id')
            ->leftJoin('p_p_p_secrets as ps','ps.id','=','ci.ppp_user_id')->whereNotNull('customers_addresses.latitude')->whereNotNull('customers_addresses.longitude')->limit(1000)

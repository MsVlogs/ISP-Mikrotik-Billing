...
Route::middleware([
    ...
    Route::middleware(EnsureBillingPort::class)->group(function () {
        ...
        Route::get('/network-inventory/olt/{device}/provisioning', [\App\Http\Controllers\OltProvisioningController::class, 'index'])->name('network-inventory.olt.provisioning');
        Route::post('/network-inventory/olt/{device}/provision', [\App\Http\Controllers\OltProvisioningController::class, 'execute'])->name('network-inventory.olt.provision');
        Route::get('/network-inventory/olt/{device}/customers', function(\Illuminate\Http\Request $request, \App\Models\NetworkInventoryDevice $device){ abort_unless($device->type==='olt',404); $q=trim((string)$request->query('q','')); $status=$request->query('status'); $mappings=\App\Models\OltOnuCustomerMapping::with(['customer','pppUser'])->where('olt_device_id',$device->id)->when($q!=='' , function($query) use ($q){ $like='%'.$q.'%'; $query->where(function($w) use ($like){ $w->where('onu_id','like',$like)->orWhere('onu_serial','like',$like)->orWhere('onu_mac','like',$like)->orWhere('pon_port','like',$like)->orWhereHas('pppUser',fn($p)=>$p->where('username','like',$like))->orWhereHas('customer',fn($c)=>$c->where('customer_name','like',$like)->orWhere('customer_unique_id','like',$like)->orWhere('mobile','like',$like)); }); })->when(in_array($status,['online','offline','unknown'],true),fn($query)=>$query->where('status',$status))->orderBy('id')->get(); return view('xlink.olt-customers',compact('device','mappings','q','status')); })->name('network-inventory.olt.customers');
